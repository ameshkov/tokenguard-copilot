...
import { buildRequestBody, parseCustomFields } from './build-request-body.js';
import type { ChatContext, OpenAIMessage, OpenAITool } from './chat-types.js';
...
describe('buildRequestBody', () => {
  ...
  it('builds basic request body', () => {
    const body = buildRequestBody(messages, baseContext);
  ...
  it('omits stream_options when not streaming', () => {
    ...
    const body = buildRequestBody(messages, ctx);
...
describe('parseCustomFields', () => {
  it('parses string, number, boolean, json types', () => {
    const result = parseCustomFields(
      JSON.stringify([
        { property: 'user', type: 'string', value: 'test-user' },
        { property: 'max_tokens', type: 'number', value: '4096' },
  ...
  it('skips field with empty property', () => {
    const result = parseCustomFields(
      JSON.stringify([{ property: '', type: 'string', value: 'orphan' }]),
    );
  ...
  it('handles invalid JSON input', () => {
    expect(parseCustomFields('{bad}')).toEqual({});
  });

  it('handles non-array input', () => {
    expect(parseCustomFields('"string"')).toEqual({});
  });
});
